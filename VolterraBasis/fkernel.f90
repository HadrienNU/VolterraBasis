...
subroutine euler_integral(res,dt,n,B,kernel,dim_basis)
  implicit none
  integer,intent(in)::n,dim_basis
  double precision,dimension(0:n, dim_basis, dim_basis),intent(in)::B
  double precision,dimension(0:n, dim_basis),intent(out)::kernel
  double precision,dimension(dim_basis),intent(out)::res
...
subroutine midpoint_integral(res,dt,n,B,kernel,dim_basis)
  implicit none
  integer,intent(in)::n,dim_basis
...
subroutine trapz_integral(res,dt,n,B,kernel,dim_basis)
  implicit none
  integer,intent(in)::n,dim_basis
  double precision,dimension(0:n, dim_basis, dim_basis),intent(in)::B
  double precision,dimension(0:n, dim_basis),intent(out)::kernel
...
subroutine simpson_integral(res,dt,n,B,kernel,dim_basis)
  implicit none
  integer,intent(in)::n,dim_basis
  double precision,dimension(0:n, dim_basis, dim_basis),intent(in)::B
  double precision,dimension(0:n, dim_basis),intent(out)::kernel
...
subroutine kernel_first_kind_trapz(lenTraj, dim_basis, kernel, k0, B, DxB,dt)
  use lapackMod
  implicit none
  integer,intent(in)::lenTraj,dim_basis
  double precision,dimension(0:lenTraj, dim_basis),intent(out)::kernel
  double precision,dimension(dim_basis),intent(in)::k0
  double precision,dimension(0:lenTraj, dim_basis, dim_basis),intent(in)::B
  double precision,dimension(0:lenTraj, dim_basis),intent(in)::DxB
  double precision,intent(in)::dt
  double precision,dimension(dim_basis,dim_basis)::invB0
  double precision,dimension(dim_basis)::num
  integer::i
  ...
  do i=1,lenTraj !! for i in range(1, lenTraj):
     call trapz_integral(num,dt,i,B(0:i,:,:),kernel(0:i,:),dim_basis)
...
subroutine kernel_first_kind_euler(lenTraj, dim_basis, kernel,  B, DxB,dt)
  use lapackMod
  implicit none
  integer,intent(in)::lenTraj,dim_basis
  double precision,dimension(0:lenTraj, dim_basis),intent(out)::kernel
  ...
  do i=1,lenTraj-1 !! for i in range(1, lenTraj):
     call euler_integral(num,dt,i,B(1:i+1,:,:),kernel(0:i,:),dim_basis)
...
subroutine kernel_first_kind_midpoint(lenTraj, dim_basis, kernel,  B, DxB,dt)
  use lapackMod
  implicit none
  integer,intent(in)::lenTraj,dim_basis
  double precision,dimension(0:(lenTraj-1)/2, dim_basis),intent(out)::kernel
  double precision,dimension(0:lenTraj, dim_basis, dim_basis),intent(in)::B
  double precision,dimension(0:lenTraj, dim_basis),intent(in)::DxB
  double precision,intent(in)::dt
  double precision,dimension(dim_basis,dim_basis)::invB0
  ...
  do i=1,lenTraj/2-1 !! for i in range(1, lenTraj):
     call midpoint_integral(num,2*dt,i,B(0:2*i,:,:),kernel(0:i,:),dim_basis)
...
subroutine kernel_first_kind_simpson(lenTraj, dim_basis, kernel, k0, B, DxB,dt)
  use lapackMod
  implicit none
  integer,intent(in)::lenTraj,dim_basis
  double precision,dimension(0:lenTraj, dim_basis),intent(out)::kernel
  ...
  do i=2,lenTraj
     call simpson_integral(num,dt,i,B(0:i,:,:),kernel(0:i,:),dim_basis)
     kernel(i,:)=-1*matmul(invB0,num+DxB(i,:))
  end do
...
subroutine kernel_second_kind(lenTraj, dim_basis, kernel, k0, B0, Bdot, DxBdot,dt)
  use lapackMod
  implicit none
  integer,intent(in)::lenTraj,dim_basis
  double precision,dimension(0:lenTraj, dim_basis),intent(out)::kernel
  ...
  double precision,dimension(0:lenTraj, dim_basis),intent(in)::DxBdot
  double precision,intent(in)::dt
  double precision,dimension(dim_basis,dim_basis)::invB0
  double precision,dimension(dim_basis)::num
  integer::i
  ...
  do i=1,lenTraj !! for i in range(1, lenTraj):
     call trapz_integral(num,dt,i,Bdot(0:i,:,:),kernel(0:i,:),dim_basis)
...
subroutine kernel_second_kind_simpson(lenTraj, dim_basis, kernel, k0, B0, Bdot, DxBdot,dt)
  use lapackMod
  implicit none
  integer,intent(in)::lenTraj,dim_basis
  double precision,dimension(0:lenTraj, dim_basis),intent(out)::kernel
  ...
  do i=2,lenTraj !! for i in range(1, lenTraj):
     call simpson_integral(num,dt,i,Bdot(0:i,:,:),kernel(0:i,:),dim_basis)
     kernel(i,:)=matmul(invB0,num+DxBdot(i,:))
  end do
