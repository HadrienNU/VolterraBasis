...
axs.grid()
axs.plot(model.time_kernel, kernel[:, 0, 0], "-", label="Memory Kernel")
for type in ["exp", "sech", "gaussian"]:
    print("Fit: " + str(type))
    params = vb.memory_fit(model.time_kernel.ravel(), kernel[:, 0, 0], type=type)
    print(params)
    axs.plot(model.time_kernel, vb.memory_fit_eval(model.time_kernel, params), "-x", label="Fit " + str(type))
axs.legend(loc="best")
