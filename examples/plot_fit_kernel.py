...
axs.grid()
axs.plot(model.kernel["time"], kernel[:, 0, 0], "-", label="Memory Kernel")
for type in ["exp", "sech", "gaussian"]:
    print("Fit: " + str(type))
    params = vb.memory_fit(model.kernel["time"], kernel[:, 0, 0], type=type)
    print(params)
    axs.plot(model.kernel["time"], vb.memory_fit_eval(model.kernel["time"], params), "-x", label="Fit " + str(type))
axs.legend(loc="best")
